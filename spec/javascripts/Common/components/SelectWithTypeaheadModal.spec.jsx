...
const defaultProps = {
  ...
  name: 'input_name',
  cells,
  fetchItemsPath: '/ships/rocinante/crew'
}
...
const mountWrapper = (props) => mount(<SelectWithTypeaheadModal {...{ ...defaultProps, ...props }} />)
...
describe('with more than 20 items', () => {
  ...
  it('should be able to show a modal', () => {
    const wrapper = mountWrapper({ items })
    expect(wrapper.find('TypeaheadTableModal').props().isOpen).toBe(false)

    openModal(wrapper)
    expect(wrapper.find('TypeaheadTableModal').props().isOpen).toBe(true)
